...
def Windef():
  def EnableWindef():
    os.system("@echo off & powershell -Command \"Start-Process -Wait -Verb RunAs PowerShell -ArgumentList '-NoProfile -ExecutionPolicy Bypass -Command Set-MpPreference -DisableRealtimeMonitoring $true'\"")
    notification.notify( 
    title= "EWT",
  ...
  def DisableWindef():
    os.system("@echo off & powershell -Command \"Start-Process -Wait -Verb RunAs PowerShell -ArgumentList '-NoProfile -ExecutionPolicy Bypass -Command Set-MpPreference -DisableRealtimeMonitoring $false'\"")
    notification.notify( 
    title= "EWT",
...
def CleanerScript():
  os.system('start @echo off & del /q /s /f %temp%\\*.* & del /q /s /f C:\\Windows\\Temp\\* & rd /s /q "C:\\Users\\%username%\\AppData\\Local\\Google\\Chrome\\User Data\\Default\\Cache" & del /q /s /f C:\\Windows\\Logs\\* & del /q /s /f C:\\Windows\\SoftwareDistribution\\Download\\*.* & del /q /s /f %windir%\\Installer\\*.*')
  notification.notify( 
  title= "EWT",
...
def UsernitResetScript():
  os.system('@echo off & reg add "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon" /v Userinit /t REG_SZ /d "C:\\Windows\\system32\\userinit.exe,"')
  notification.notify( 
    title= "EWT",
...
def UpdatePCScript():
  import os
  os.system('start assets\\update.bat')
...
def VirusScannerScript():
  ...
  # Define virus signatures (this could be much more extensive in a real-world scenario)
  virus_signatures = [
      b'malicious_code_1',
      b'malicious_code_2',
  ...
  def browse_directory():
      ...
      if directory_path:
          ...
          infected_files = scan_directory(directory_path)
          if infected_files:
              result_text.insert(tk.END, "Infected files found:\n")
  ...
  label = tk.Label(root, text="Select a directory to scan:")
  label.pack()
